...
public class PriorityPreemptive extends CPUScheduler{
    ...
    public void process(){
        Collections.sort(this.getRows(), (Object o1, Object o2) -> {
            if (((Row) o1).getArrivalTime() == ((Row) o2).getArrivalTime()) return 0;
            else if (((Row) o1).getArrivalTime() < ((Row) o2).getArrivalTime()) return -1;
            else return 1;
        });
        ...
        while (!rows.isEmpty()){
            ...
            for (Row row : rows){
                if (row.getArrivalTime() <= time) availableRows.add(row);
            }
            Collections.sort(availableRows, (Object o1, Object o2) -> {
                if (((Row) o1).getPriorityLevel()== ((Row) o2).getPriorityLevel()) return 0;
                else if (((Row) o1).getPriorityLevel() < ((Row) o2).getPriorityLevel()) return -1;
                else return 1;
            });
        ...
        for (Row row : this.getRows()){
            ...
            for (Event event : this.getTimeline()){
                if (event.getProcessName().equals(row.getProcessName())){
                    ...
                    else row.setWaitingTime(event.getStartTime() - row.getArrivalTime());
